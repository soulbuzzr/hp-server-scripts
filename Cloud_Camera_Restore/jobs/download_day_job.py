...
if len(sys.argv) != 2:
    print(f"Usage: {Path(sys.argv[0]).name} <date YYYY-MM-DD> ")
    sys.exit(1)
...
async def main():
    ...
    console.print(
        f"Found {len(sessions)} worker session(s)"
    )
    console.print()

    search_session = sessions[0]

    console.print("Searching Telegram for day's recordings...")
    console.print()

    prefix = f"{DOWNLOAD['camera']}_{DOWNLOAD['date']}_"
